...
\begin{itemize}
    \item \textbf{Relacje między listami w SharePoint:}
          \begin{itemize}
              ...
              \customnote{No tutaj nie tylko lookup ale też filter było wykorzystywane. Bo lookup znajduje ci pierwszy pasujący wiersz a filter wypluwa ci listę pasujących elementów. I my robiliśmy te śmieszne zagnieżdżenia.}
          \end{itemize}
    ...
    \item \textbf{Wykorzystanie przepływów podrzędnych w Power Automate:}
          \begin{itemize}
            ...
            %    \customnote{O child flow jest trochę opisane tutaj} \ref{childflow}. \customnote{Ale ostatnio chyba i tak się zbugowało nawet child flow i coś wyszło z tym delayem więc chyba wypadałoby coś dopowiedzieć}
            \customnote{Imo to jest git, bo tam tylko mówimy, że używamy a wyjaśnienie masz tutaj. Conajwyżej można dodać refa do tego miejsca.}
          \end{itemize}


    \item \textbf{Ustawienia lokalne - średniki i przecinki:}
          \begin{itemize}
            \item \customnote{TO BYŁ TYLKO PROBLEM JAK WRZUCAŁEŚ FUNKCJE Z CHATA ALBO Z FORUM XDD ja bym to wywalił. w sensie mogłoby zostać ale trochę do pizdy jest to napisane moim zdaniem. Możemy zapytać pikeja co sadzi}
